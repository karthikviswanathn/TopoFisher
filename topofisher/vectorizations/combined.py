...
class CombinedVectorization(nn.Module):
    ...
    Supports both standard layers (TopKLayer) and MMA layers (MMATopKLayer, MMAKernelLayer).
    """
    ...
    def forward(self, all_diagrams: List[List[torch.Tensor]], 
        ...
        Args:
            all_diagrams: List[hom_dim][sample_idx] of diagrams or MMA objects
                         e.g., [[H0_sample0, H0_sample1, ...], [H1_sample0, H1_sample1, ...]]
            field: Optional field tensor for MMA layers (shape: n_samples, H, W)
            gradient: Optional gradient tensor for MMA layers (shape: n_samples, H, W)
        ...
        for hom_dim_idx, layer in enumerate(self.layers):
            ...
            # Check if layer needs field and gradient (MMA layers)
            if hasattr(layer, 'homology_dimension'):
                # MMA layer: pass field and gradient
                if field is None or gradient is None:
                    raise ValueError(f"Layer {layer.__class__.__name__} requires field and gradient")
                features = layer(diagrams_for_dim, field, gradient)
            else:
                # Standard layer: only diagrams
