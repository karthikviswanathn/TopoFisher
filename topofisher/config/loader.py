...
MODIFIED FOR MMA BRANCH: Uses FisherPipeline (from core.pipeline) instead of BasePipeline.
FisherPipeline automatically handles MMA filtrations that require gradient input.
"""
import os
...
def create_pipeline_from_config(config: PipelineYAMLConfig):
    ...
    MODIFIED FOR MMA: Uses FisherPipeline which automatically handles
    MMA filtrations (that require gradient input) and standard filtrations.

    Args:
        config: Pipeline configuration

    Returns:
        Pipeline instance (FisherPipeline or cached variant)
    ...
    # No cache - use FisherPipeline (handles both MMA and standard filtrations)
    # FisherPipeline automatically detects if filtration needs gradient input
